/**
 * Remove "zero" character from all users
 * Usage:
 *   npx ts-node scripts/remove_zero_character.ts --serviceAccount=./secrets/kanji-study-50c28-firebase-adminsdk-fbsvc-5267decc62.json --dryRun
 *   npx ts-node scripts/remove_zero_character.ts --serviceAccount=./secrets/kanji-study-50c28-firebase-adminsdk-fbsvc-5267decc62.json
 *
 * The script will:
 *  - Read all documents in `users` collection
 *  - Check if user has "zero" character in their characters subcollection
 *  - Remove "zero" character from the array
 *  - Update the document
 *
...
async function removeZeroCharacter() {
  ...
  for (const doc of usersSnap.docs) {
    ...
    if (charactersSnap.exists) {
      ...
      const hasZero = characters.some((char: any) => char.id === 'zero');

      if (hasZero) {
        const filteredCharacters = characters.filter((char: any) => char.id !== 'zero');
    ...
    // Check if "zero" is equipped in the main user document
