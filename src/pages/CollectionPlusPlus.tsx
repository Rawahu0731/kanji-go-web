...
export default function CollectionPlusPlus() {
  const { state, getCollectionPlusPlusEffect } = useGamification();
  const list = state.collectionPlusPlus || [];
  ...
  const filtered = useMemo(() => {
    ...
    if (showOnlyMaxed) items = items.filter(e => e.plus >= 100);
    ...
    <div className="collection-plus-container">
      <header className="collection-plus-header">
        ...
        <div className="collection-stats">
          ...
          {(() => {
            const eff = getCollectionPlusPlusEffect();
            if (!eff) return null;
      ...
      <div className="tiles">
        ...
        {filtered.map(item => (
          <div key={item.kanji} className={`tile ${ (item.plus||0) >= 100 ? 'maxed' : ''}`}>
